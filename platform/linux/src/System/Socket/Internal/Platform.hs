...
wait (Socket mfd) threadWait threadWaitSTM
  | rtsSupportsBoundThreads =
      bracketOnError
        ...
        snd
        (atomically . fst)
        `catch` (const (throwIO eBadFileDescriptor) :: IOError -> IO ())
  | otherwise = do
      m <- newEmptyMVar
      bracketOnError
        ( withMVar mfd $ \fd -> do
...
foreign import ccall unsafe "hs_sendto"
  c_sendto
    :: Fd -> Ptr a -> CSize -> MessageFlags -> Ptr b -> CInt -> Ptr CInt -> IO CSSize
...
foreign import ccall safe "getnameinfo"
  c_getnameinfo
    :: Ptr a -> CInt -> CString -> CInt -> CString -> CInt -> CInt -> IO CInt
