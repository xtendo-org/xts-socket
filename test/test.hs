...
group01 = testGroup "connect" [testGroup "Inet/Stream/TCP" t1]
 where
  t1 =
    [ testCase "connect to closed port on inetLoopback" $
        bracket
          ...
          ( \s -> do
              ...
              case r of
                Left e
                  | e == eConnectionRefused -> return ()
                  | otherwise -> throwIO e
                Right () -> assertFailure "connection should have failed"
          )
    , testCase "connect to closed port on inetNone" $
        bracket
          (socket :: IO (Socket Inet Stream TCP))
          close
          ( \s -> do
    ...
    , testCase "connect closed socket" $
        bracket
          ( do
              server <- socket :: IO (Socket Inet Stream TCP)
              client <- socket :: IO (Socket Inet Stream TCP)
          ...
          `catch` \e -> case e of
            _
              | e == eBadFileDescriptor -> return ()
              | otherwise -> assertFailure "expected eBadFileDescriptor"
    , testCase "connect already connected socket" $
        bracket
          ( do
              server <- socket :: IO (Socket Inet Stream TCP)
              client <- socket :: IO (Socket Inet Stream TCP)
...
group02 =
  testGroup
    ...
    [ testGroup
        "Inet/Datagram/UDP"
        [ testCase "listen on bound socket"
            $ bracket
              (socket :: IO (Socket Inet Datagram UDP))
              close
            $ \sock -> do
...
group03 =
  testGroup
    ...
    [ testGroup
        "Inet/Stream/TCP"
        [ testCase "send and receive a chunk" $
            bracket
              ( do
                  server <- socket :: IO (Socket Inet Stream TCP)
                  client <- socket :: IO (Socket Inet Stream TCP)
        ...
        , testCase "recv empty bytestring when peer disconnected gracefully" $
            bracket
              (socket :: IO (Socket Inet Stream TCP))
              close
              ( \server -> do
        ...
        , testCase "trigger ePipe exception" $
            bracket
              ( do
                  server <- socket :: IO (Socket Inet Stream TCP)
                  client <- socket :: IO (Socket Inet Stream TCP)
    ...
    , testGroup
        "Inet/Datagram/UDP"
        [ testCase "send and receive a datagram" $
            bracket
              ( do
        ...
        [ testCase "sendAll and receiveAll a 128MB chunk" $
            bracket
              ...
              ( \(server, client) -> do
                  ...
                  when
                    (fromIntegral sent /= LBS.length msg)
...
group98 =
  testGroup
    "getAddress"
    [ testCase "getAddress after bind" $
        bracket
...
group99 =
  testGroup
    ...
    [ testCase "getAddrInfo \"127.0.0.1\" \"80\"" $ do
        ...
        when (inetAddress sa /= inetLoopback) $
          assertFailure "expected loopback address"
        when (inetPort sa /= 80) $ assertFailure "expected port 80"
    , testCase "getAddrInfo \"\" \"\"" $
        void
          (getAddressInfo Nothing Nothing mempty :: IO [AddressInfo Inet Stream TCP])
          `catch` \e -> case e of
