...
unsafeSendTo s bufPtr bufSize flags addrPtr addrSize =
  tryWaitRetryLoop
    s
    waitWrite
    (\fd -> c_sendto fd bufPtr (fromIntegral bufSize) flags addrPtr addrSize)
...
unsafeReceiveFrom s bufPtr bufSize flags addrPtr addrSizePtr =
  tryWaitRetryLoop
    s
    waitRead
    (\fd -> c_recvfrom fd bufPtr bufSize flags addrPtr addrSizePtr)
    ...
    mi <- withMVar mfd $ \fd -> alloca $ \errPtr -> do
      ...
      if i < 0
        then do
          ...
          unless
            (err == eWouldBlock || err == eAgain || err == eInterrupted)
            (throwIO err)
          return Nothing
        else
